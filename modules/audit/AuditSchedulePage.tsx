...
export const AuditSchedulePage: React.FC<AuditSchedulePageProps> = ({ 
  ...
  const [viewMode, setViewMode] = useState<'list' | 'calendar'>('list'); // State toggle view
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
     e.stopPropagation();
     if (window.confirm("Đại ca có chắc muốn xóa kế hoạch này không?")) {
  ...
  // Filter Auditors based on selected Organization
  ...
  const columns: ColumnDefinition<KeHoachDanhGia>[] = [
      { 
          ...
          render: (val, item) => (
              <div>
                  <div className="font-medium text-gray-800 dark:text-gray-200">{val}</div>
                  <div className="text-xs text-gray-500 mt-1">{item.loai_danh_gia}</div>
              </div>
          )
      },
      {
          key: 'to_chuc_danh_gia_id',
          header: 'Tổ chức đánh giá',
          visible: true,
          render: (val) => {
              const org = masterData.toChucDanhGia?.find(o => o.id === val);
              return <span className="text-sm">{org ? org.ten : '--'}</span>;
          }
      },
      {
          key: 'truong_doan_id',
          ...
          visible: true,
          render: (val) => {
              const lead = masterData.auditors?.find(a => a.id === val);
              return lead ? (
                 <div className="flex items-center gap-1 text-sm">
                    <UserCheck size={14} className="text-blue-500"/> {lead.ten}
                 </div>
              ) : '--';
          }
      },
      {
          key: 'thoi_gian_du_kien_start',
          header: 'Thời gian',
          visible: true,
          render: (_, item) => (
             <span className="text-xs">
                 {item.thoi_gian_du_kien_start ? format(new Date(item.thoi_gian_du_kien_start), 'dd/MM/yyyy') : '?'} 
                 {' - '} 
                 {item.thoi_gian_du_kien_end ? format(new Date(item.thoi_gian_du_kien_end), 'dd/MM/yyyy') : '?'}
             </span>
          )
      },
      { 
          ...
          header: 'Trạng thái', 
          visible: true, 
          render: (val) => getStatusBadge(val as TrangThaiKeHoach) 
      },
      {
          key: 'id',
          header: 'Thao tác',
          ...
          render: (_, item) => (
             <div className="flex gap-1">
                ...
                <Button variant="ghost" size="icon" onClick={(e) => handleDelete(item.id, e)}>
  ...
  const renderFilters = (
      <div className="flex bg-gray-100 dark:bg-slate-800 rounded p-1 mr-2 border border-gray-200 dark:border-slate-700">
         <button 
           ...
           className={`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-medium transition-all ${viewMode === 'list' ? 'bg-white dark:bg-slate-600 shadow text-primary' : 'text-gray-500 hover:text-gray-700'}`}
           title="Xem dạng bảng"
         >
         ...
         <button 
           ...
           className={`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-medium transition-all ${viewMode === 'calendar' ? 'bg-white dark:bg-slate-600 shadow text-primary' : 'text-gray-500 hover:text-gray-700'}`}
  ...
  return (
    <div className="h-full flex flex-col animate-fade-in -mx-4 md:mx-0 relative">
        <div className="flex-1 overflow-hidden h-full rounded-lg border border-gray-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900 flex flex-col">
            {viewMode === 'calendar' ? (
                <>
                    {/* Custom Header for Calendar View to match DataTable Header */}
                    <div className="p-3 border-b border-gray-200 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/30 rounded-t-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-3 shrink-0">
                        <div className="flex items-center gap-3">
                            <h3 className="font-bold text-gray-800 dark:text-gray-100">Lịch đánh giá (Có lịch Âm)</h3>
                            {renderFilters}
                        </div>
                        <Button onClick={handleAddNew} leftIcon={<Plus size={16} />} className="shadow-sm h-9 text-sm">
                            Lập kế hoạch mới
                        </Button>
                    </div>
                    <div className="flex-1 overflow-hidden p-0 relative">
                        <AuditCalendar auditPlans={auditPlans} onEventClick={handleCalendarEventClick} />
                    </div>
                </>
            ) : (
                <DataTable 
                    title="Danh sách Chương trình đánh giá"
                    data={auditPlans}
                    columns={columns}
                    onRowClick={handleEdit}
                    filters={renderFilters}
                    actions={
                        <Button onClick={handleAddNew} leftIcon={<Plus size={16} />} className="shadow-sm">
                            Lập kế hoạch mới
                        </Button>
                    }
                />
        ...
        {/* Drawer Form (Slide-in) */}
        {isDrawerOpen && (
            <div className="fixed inset-0 z-50 flex justify-end">
                <div className="absolute inset-0 bg-black/30 backdrop-blur-sm transition-opacity" onClick={() => setIsDrawerOpen(false)} />
                <div className="w-full md:max-w-2xl bg-white dark:bg-slate-900 h-full shadow-2xl relative animate-slide-in-right flex flex-col transition-colors border-l border-gray-200 dark:border-slate-800">
                    ...
                    <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/50">
                        <h2 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                            ...
                            {editingPlan.id ? 'Cập nhật kế hoạch đánh giá' : 'Lập kế hoạch đánh giá mới'}
                        </h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2">
                                    ...
                                    <input 
                                        ...
                                        placeholder="VD: Đánh giá nội bộ lần 1 năm 2024"
                                        value={editingPlan.ten_ke_hoach || ''}
                                        onChange={(e) => setEditingPlan({...editingPlan, ten_ke_hoach: e.target.value})}
                                        autoFocus
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <SearchableSelect 
                                        ...
                                        value={editingPlan.loai_danh_gia}
                                        onChange={(val) => setEditingPlan({...editingPlan, loai_danh_gia: String(val)})}
                                ...
                                <div>
                                    ...
                                    <select 
                                        className={inputClass}
                                        value={editingPlan.trang_thai}
                                        onChange={(e) => setEditingPlan({...editingPlan, trang_thai: e.target.value as TrangThaiKeHoach})}
                                    >
                        ...
                        {/* Đơn vị thực hiện */}
                        <div className="space-y-4">
                             <h3 className="text-sm font-bold text-gray-500 uppercase border-b dark:border-slate-800 pb-1 flex items-center gap-2">
                                <Briefcase size={14}/> Đơn vị thực hiện
                             </h3>
                             <div className="p-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-gray-100 dark:border-slate-700 grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className={labelClass}>Tổ chức đánh giá</label>
                                    <SearchableSelect 
                                        options={auditOrgOptions}
                                        value={editingPlan.to_chuc_danh_gia_id}
                                        onChange={(val) => setEditingPlan({
                                            ...editingPlan, 
                                            to_chuc_danh_gia_id: String(val),
                                            truong_doan_id: '' // Reset lead when org changes
                                        })}
                                        placeholder="-- Chọn tổ chức --"
                                    />
                                </div>
                                <div>
                                    <label className={labelClass}>Trưởng đoàn đánh giá</label>
                                    <SearchableSelect 
                                        options={filteredAuditors}
                                        value={editingPlan.truong_doan_id}
                                        onChange={(val) => setEditingPlan({...editingPlan, truong_doan_id: String(val)})}
                                        placeholder={editingPlan.to_chuc_danh_gia_id ? "-- Chọn trưởng đoàn --" : "-- Chọn tổ chức trước --"}
                        ...
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold text-gray-500 uppercase border-b dark:border-slate-800 pb-1 flex items-center gap-2">
                                <CalendarDays size={14}/> Thời gian & Phạm vi
                             </h3>
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => setEditingPlan({...editingPlan, thoi_gian_du_kien_start: e.target.value})}
                                        />
                                    </div>
                                    <div className="relative">
                                        <Calendar size={16} className="absolute left-3 top-2.5 text-gray-400"/>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <textarea 
                                        className="w-full p-2 pl-9 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-2 ring-primary/20 outline-none transition-all text-sm min-h-[80px]"
                                        placeholder="Xác định sự phù hợp của hệ thống quản lý..."
                                        value={editingPlan.muc_tieu || ''}
                                        onChange={(e) => setEditingPlan({...editingPlan, muc_tieu: e.target.value})}
                                    />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <textarea 
                                        className="w-full p-2 pl-9 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-2 ring-primary/20 outline-none transition-all text-sm min-h-[80px]"
                                        placeholder="Các phòng ban, quy trình liên quan..."
                                        value={editingPlan.pham_vi || ''}
                                        onChange={(e) => setEditingPlan({...editingPlan, pham_vi: e.target.value})}
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="p-6 border-t border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-900 sticky bottom-0 flex justify-end gap-3">
                        ...
                        <Button onClick={handleSave} leftIcon={<Save size={16} />}>Lưu kế hoạch</Button>
                    </div>
                </div>
            </div>
        )}
