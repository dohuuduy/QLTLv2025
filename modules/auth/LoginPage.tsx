...
import { ShieldCheck, Mail, Lock, Loader2, Wrench, Key, Eye, EyeOff, CheckCircle } from 'lucide-react';
  ...
  useEffect(() => {
      const initPage = async () => {
          ...
          if (window.location.hash && window.location.hash.includes('access_token')) {
              setIsLoading(true);
              // Đặt thông báo giả lập trong lúc chờ App.tsx xử lý event onAuthStateChange
              setError("Đang xác thực tài khoản từ Email...");
              return; 
          }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-950 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-800 p-8 relative z-10 animate-in fade-in zoom-in-95 duration-300">
        ...
        {/* --- FORM LOGIN OR VERIFYING STATE --- */}
        {isLoading && error && error.includes('xác thực') ? (
            <div className="flex flex-col items-center justify-center py-8 space-y-4">
                <Loader2 className="w-10 h-10 text-blue-600 animate-spin" />
                <p className="text-gray-600 dark:text-gray-300 font-medium animate-pulse">{error}</p>
                <p className="text-xs text-gray-400">Đang đăng nhập vào hệ thống...</p>
            </div>
        ) : (
