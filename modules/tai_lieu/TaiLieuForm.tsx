...
import { Info, Calendar, UserCheck, FileType, Paperclip, Trash2, Link as LinkIcon, FileText, FileSpreadsheet, File, RefreshCw, Lock, Unlock, Layers, Tag, UploadCloud, AlertCircle, Save, PenTool, Search as SearchIcon, FileSignature, GitCommit, ArrowRight, Fingerprint, FileBox } from 'lucide-react';
...
export const TaiLieuForm: React.FC<TaiLieuFormProps> = ({ initialData, onSave, onCancel, masterData, fullList = [] }) => {
  ...
  useEffect(() => {
      ...
      if (formData.tai_lieu_cha_id) {
          ...
          // For simplicity, just prefix + random or sequential if backend supported
      }
  ...
  const getFileTypeConfig = (type: string) => {
    switch(type) {
      case 'pdf': return { icon: <FileType size={18} />, color: 'text-red-600', bg: 'bg-red-50' };
      case 'doc': return { icon: <FileText size={18} />, color: 'text-blue-600', bg: 'bg-blue-50' };
      case 'excel': return { icon: <FileSpreadsheet size={18} />, color: 'text-emerald-600', bg: 'bg-emerald-50' };
      case 'link': return { icon: <LinkIcon size={18} />, color: 'text-indigo-600', bg: 'bg-indigo-50' };
      default: return { icon: <File size={18} />, color: 'text-slate-600', bg: 'bg-slate-100' };
    }
  };
  ...
  // Styles
  const inputClass = "w-full h-10 px-3 rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 ring-primary/20 focus:border-primary outline-none transition-all text-sm disabled:opacity-60 disabled:cursor-not-allowed";
  const textareaClass = "w-full p-3 rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 ring-primary/20 focus:border-primary outline-none transition-all text-sm resize-none";
  const labelClass = "text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1.5 block flex items-center gap-1.5 tracking-wide";
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-slate-950">
      <form id="document-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-6">
        ...
        <div className="mb-6 bg-white dark:bg-slate-900 p-5 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                <div className="md:col-span-8">
                    <label className={labelClass}><FileBox size={14} className="text-blue-500"/> Tên tài liệu <span className="text-red-500">*</span></label>
                ...
                <div className="md:col-span-4">
                    <label className={labelClass}><Fingerprint size={14} className="text-purple-500"/> Mã tài liệu <span className="text-red-500">*</span></label>
                    <div className="relative group">
                        <input 
                            ...
                            className="w-full h-11 px-4 rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-lg font-mono font-bold text-blue-700 dark:text-blue-400 focus:ring-2 ring-blue-500/20 focus:border-blue-500 outline-none transition-all uppercase" 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className={cardClass}>
                <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 border-b border-gray-100 dark:border-slate-800 pb-3 mb-4">
                    <Layers size={16} className="text-blue-500"/> Phân loại & Phạm vi
                </h4>
                <div className="space-y-4 flex-1">
                    ...
                    <div>
                        ...
                        <SearchableSelect options={availableParents} value={formData.tai_lieu_cha_id} onChange={(val) => handleSelectChange('tai_lieu_cha_id', val)} placeholder="-- Không có --" />
                    </div>
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                            {masterData.tieuChuan.map(item => {
                                ...
                                return (
                                    <button key={item.id} type="button" onClick={() => toggleTieuChuan(item.ten)} className={`px-2.5 py-1.5 rounded-md text-[11px] font-semibold border transition-all flex items-center gap-1.5 ${isSelected ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800 shadow-sm' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-slate-700 hover:border-gray-300'}`}>
            ...
            {/* --- BLOCK 3: CONTROL & VALIDITY (Center) --- */}
            <div className={cardClass}>
                <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 border-b border-gray-100 dark:border-slate-800 pb-3 mb-4">
                ...
                <div className="space-y-4 flex-1">
                    ...
                    <div className={`p-3 rounded-xl border transition-all ${isReviewEnabled ? 'bg-orange-50 border-orange-200 dark:bg-orange-900/10 dark:border-orange-800' : 'bg-gray-50 dark:bg-slate-800 border-gray-100 dark:border-slate-700'}`}>
                        ...
                        {isReviewEnabled && (
                            <div className="flex items-end gap-2 animate-in fade-in">
                                ...
                                <div className="flex-[2]">
                                    ...
                                    <span className="text-[9px] text-orange-600/70 block text-center mt-0.5">Lần tới</span>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className={cardClass}>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    <div className="bg-gray-50 dark:bg-slate-800/50 rounded-lg border border-dashed border-gray-300 dark:border-slate-700 p-3 mb-3 group hover:border-blue-400 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-all">
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-2 max-h-[200px]">
                                ...
                                return (
                                    <div key={file.id} className="flex items-center gap-2 p-2 rounded-lg border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm group hover:border-blue-300 transition-all">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <span className="text-[10px] text-gray-400">{format(new Date(file.ngay_upload), 'dd/MM/yyyy')}</span>
                                        </div>
                                        <button type="button" onClick={() => removeFile(file.id)} className="p-1.5 text-gray-400 hover:text-red-500 rounded transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                                    </div>
                                );
                            })
                        ) : (
        ...
        {/* --- BLOCK 5: CONTENT DESCRIPTION (Full Width) --- */}
        <div className="mb-6 bg-white dark:bg-slate-900 p-5 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
            <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 border-b border-gray-100 dark:border-slate-800 pb-3 mb-4">
                <Info size={16} className="text-blue-500"/> Nội dung tóm tắt
            </h4>
            <textarea name="mo_ta_tom_tat" value={formData.mo_ta_tom_tat} onChange={handleChange} className={`${textareaClass} min-h-[120px] text-base leading-relaxed`} placeholder="Mô tả phạm vi áp dụng, mục đích và các nội dung chính của tài liệu..." />
        </div>

        {/* --- BLOCK 6: WORKFLOW MATRIX (Process Flow) --- */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
            <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 mb-6">
            ...
            <div className="relative flex flex-col md:flex-row items-start justify-between gap-6">
                {/* Connecting Line */}
                <div className="absolute top-8 left-0 w-full h-0.5 bg-gray-100 dark:bg-slate-800 hidden md:block -z-0"></div>

                {/* Step 1 */}
                <div className="relative z-10 flex-1 w-full">
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                            <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 flex flex-col items-center justify-center border border-blue-100 dark:border-blue-800 shadow-sm shrink-0">
                                <PenTool size={20} className="mb-1"/>
                                <span className="text-[10px] font-bold uppercase">Soạn thảo</span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Người thực hiện</label>
                                <SearchableSelect options={drafterOptions} value={formData.id_nguoi_soan_thao} onChange={(val) => handleSelectChange('id_nguoi_soan_thao', val)} placeholder="Chọn nhân sự..." className="w-full" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {/* Arrow */}
                <div className="hidden md:flex items-center justify-center pt-5 text-gray-300">
                    <ArrowRight size={24} />
                </div>
                ...
                <div className="relative z-10 flex-1 w-full">
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                            <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 flex flex-col items-center justify-center border border-indigo-100 dark:border-indigo-800 shadow-sm shrink-0">
                                <SearchIcon size={20} className="mb-1"/>
                                <span className="text-[10px] font-bold uppercase">Xem xét</span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Người kiểm tra</label>
                                <SearchableSelect options={reviewerOptions} value={formData.id_nguoi_xem_xet} onChange={(val) => handleSelectChange('id_nguoi_xem_xet', val)} placeholder="Chọn nhân sự..." className="w-full" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative z-10 flex-1 w-full">
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                            <div className="w-16 h-16 rounded-2xl bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 flex flex-col items-center justify-center border border-green-100 dark:border-green-800 shadow-sm shrink-0">
                                <FileSignature size={20} className="mb-1"/>
                                <span className="text-[10px] font-bold uppercase">Phê duyệt</span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Người duyệt</label>
                                <SearchableSelect options={approverOptions} value={formData.id_nguoi_phe_duyet} onChange={(val) => handleSelectChange('id_nguoi_phe_duyet', val)} placeholder="Chọn lãnh đạo..." className="w-full" />
                            </div>
                        </div>
                    </div>
                </div>
      ...
      <div className="flex justify-end gap-3 mt-4 pt-4 px-6 pb-4 border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            <Button onClick={() => handleSubmit()} leftIcon={<Save size={16} />} className="bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20 px-6">Lưu dữ liệu</Button>
      </div>
    </div>
  );
};
