...
import { SearchableSelect } from '../../components/ui/SearchableSelect'; // NEW
...
export const UserManager: React.FC<UserManagerProps> = ({ users, departments, onUpdate }) => {
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm('Đại ca có chắc muốn xóa nhân sự này khỏi hệ thống?')) {
  ...
  const handleSave = () => {
    ...
    if (editingUser.id) {
      // Update
      onUpdate(users.map(u => u.id === editingUser.id ? { ...u, ...editingUser, roles } as NhanSu : u));
    } else {
      // Add
    ...
    { 
      ...
      render: (_, item) => (
        <div className="flex items-center gap-3">
           <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center text-gray-600 dark:text-gray-300 font-bold text-sm">
              {item.ho_ten.charAt(0)}
           </div>
           <div>
             <div className="font-bold text-gray-800 dark:text-gray-200">{item.ho_ten}</div>
          ...
          <div className="w-full max-w-md bg-white dark:bg-slate-900 h-full shadow-2xl relative animate-slide-in-right flex flex-col transition-colors border-l border-gray-200 dark:border-slate-800">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-800">
              <h2 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                ...
                {editingUser.id ? 'Cập nhật nhân sự' : 'Thêm nhân sự mới'}
              </h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div className="col-span-3">
                    <label className="text-xs font-semibold mb-1 block">Họ và tên <span className="text-red-500">*</span></label>
                    <input 
                      ...
                      onChange={e => setEditingUser({...editingUser, ho_ten: e.target.value})}
                      placeholder="Nguyễn Văn A"
                    />
                  </div>
                  <div className="col-span-1">
                    <label className="text-xs font-semibold mb-1 block">Thứ tự</label>
                    <input 
                      type="number"
                      className="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-2 ring-primary/20 outline-none transition-all text-center"
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                    ...
                    <input 
                      ...
                      onChange={e => setEditingUser({...editingUser, chuc_vu: e.target.value})}
                      placeholder="Trưởng phòng..."
                    />
                  </div>
                  <div>
                    <label className="text-xs font-semibold mb-1 block">Phòng ban</label>
                    <SearchableSelect
                       options={departmentOptions}
                       value={editingUser.phong_ban}
                       onChange={(val) => setEditingUser({...editingUser, phong_ban: String(val)})}
                       placeholder="-- Chọn --"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                 <h3 className="text-sm font-bold text-gray-500 uppercase border-b dark:border-slate-800 pb-1 flex items-center gap-2">
                   <Shield size={14}/> Phân quyền hệ thống
                 </h3>
                 <div className="grid grid-cols-1 gap-2">
                   {[
                     {code: 'SOAN_THAO', label: 'Soạn thảo tài liệu', desc: 'Được phép tạo mới và chỉnh sửa tài liệu.'},
                     {code: 'XEM_XET', label: 'Xem xét / Góp ý', desc: 'Được phép review tài liệu trước khi duyệt.'},
                     {code: 'PHE_DUYET', label: 'Phê duyệt cuối cùng', desc: 'Quyền ban hành tài liệu chính thức.'},
                     {code: 'QUAN_TRI', label: 'Quản trị hệ thống (Admin)', desc: 'Toàn quyền cấu hình hệ thống.'},
                   ].map(role => (
                     <div 
                        key={role.code} 
                        className={`p-3 rounded-lg border cursor-pointer transition-all flex items-start gap-3 ${
                          editingUser.roles?.includes(role.code as UserRole) 
                            ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' 
                            : 'border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800'
                        }`}
                        onClick={() => toggleRole(role.code as UserRole)}
                     >
                        <div className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center ${
                           editingUser.roles?.includes(role.code as UserRole) ? 'bg-primary border-primary text-white' : 'border-gray-400'
                        }`}>
                           {editingUser.roles?.includes(role.code as UserRole) && <Check size={12} />}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-gray-800 dark:text-gray-200">{role.label}</p>
            ...
            <div className="p-6 border-t border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-900 sticky bottom-0 flex justify-end gap-3">
               ...
               <Button onClick={handleSave} leftIcon={<Check size={16} />}>Lưu thông tin</Button>
            </div>
          </div>
        </div>
      )}
