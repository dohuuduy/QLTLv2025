...
export const ApprovalsPage: React.FC<ApprovalsPageProps> = ({ currentUser, documents, onUpdate }) => {
  ...
  const columns: ColumnDefinition<TaiLieu>[] = [
    ...
    { key: 'ten_tai_lieu', header: 'Tên Tài Liệu', visible: true, render: (val, doc) => (<div><div className="font-medium text-gray-900 dark:text-gray-100">{val}</div><div className="text-xs text-gray-500 flex items-center gap-2 mt-0.5"><span className="bg-gray-100 dark:bg-slate-800 px-1.5 rounded border border-gray-200 dark:border-slate-700">v{doc.phien_ban}</span><span>•</span><span>{doc.loai_tai_lieu}</span></div></div>) },
    { key: 'nguoi_soan_thao', header: 'Người trình', visible: true, render: (val) => <div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-blue-100 text-blue-600 text-xs flex items-center justify-center font-bold">{String(val).charAt(0)}</div><span className="text-sm">{val}</span></div> },
    { key: 'ngay_cap_nhat_cuoi', header: 'Ngày trình', visible: true, render: (val) => <span className="flex items-center gap-1 text-gray-500 text-xs"><Clock size={12}/>{format(new Date(val), 'dd/MM/yyyy')}</span> },
    { key: 'trang_thai', header: 'Vai trò', visible: true, render: (_, doc) => { const isApprover = doc.nguoi_phe_duyet === currentUser.ho_ten; if (currentUser.roles.includes('QUAN_TRI') && doc.nguoi_phe_duyet !== currentUser.ho_ten && doc.nguoi_xem_xet !== currentUser.ho_ten) { return <span className="text-[10px] font-bold text-gray-500 bg-gray-100 border border-gray-200 px-2 py-1 rounded-full">Quản trị viên</span>; } return isApprover ? <span className="text-[10px] font-bold text-green-700 bg-green-100 border border-green-200 px-2 py-1 rounded-full shadow-sm flex w-fit items-center gap-1"><PenTool size={10}/> Phê duyệt</span> : <span className="text-[10px] font-bold text-blue-700 bg-blue-100 border border-blue-200 px-2 py-1 rounded-full shadow-sm flex w-fit items-center gap-1"><Eye size={10}/> Xem xét</span>; } },
  ...
  return (
    <div className="space-y-6 animate-fade-in h-full flex flex-col">
       ...
       <div className="flex-1 overflow-hidden rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900"><DataTable title="Danh sách chờ duyệt" data={pendingDocs} columns={columns} onRowClick={() => {}} /></div>
       <Modal isOpen={showSignModal} onClose={() => setShowSignModal(false)} title={isRejecting ? 'Xác nhận Từ chối' : `Xác nhận ${getRoleText()}`} size="md" footer={<div className="flex justify-end gap-2 w-full"><Button variant="ghost" onClick={() => setShowSignModal(false)}>Hủy bỏ</Button><Button onClick={handleConfirmAction} isLoading={isLoading} className={isRejecting ? "bg-red-600 hover:bg-red-700 text-white shadow-red-200" : "bg-green-600 hover:bg-green-700 text-white shadow-green-200"} leftIcon={isRejecting ? <XCircle size={16}/> : <CheckCircle size={16}/>}>{isRejecting ? 'Xác nhận Từ chối' : 'Xác nhận Duyệt'}</Button></div>}>
          <div className="space-y-0 divide-y divide-gray-100 dark:divide-slate-800">
             <div className="p-5 bg-gray-50/50 dark:bg-slate-800/50">
                ...
                {selectedDoc && (<div className="space-y-3"><div><label className="text-[10px] text-gray-400 block uppercase">Mã & Phiên bản</label><div className="font-mono font-bold text-primary dark:text-blue-400 text-sm">{selectedDoc.ma_tai_lieu} <span className="text-gray-400 font-normal ml-1">v{selectedDoc.phien_ban}</span></div></div><div><label className="text-[10px] text-gray-400 block uppercase">Tên tài liệu</label><div className="font-bold text-gray-800 dark:text-gray-100 text-sm">{selectedDoc.ten_tai_lieu}</div></div><div className="text-xs text-gray-600 dark:text-gray-400 italic bg-white dark:bg-slate-900 p-2 rounded border border-gray-200 dark:border-slate-700">{selectedDoc.mo_ta_tom_tat || "Không có mô tả"}</div></div>)}
             </div>
